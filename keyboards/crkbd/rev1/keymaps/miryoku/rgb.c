...
void housekeeping_task_rgb() {
    ...
    is_rgb_enabled = timer_elapsed32(rgb_timer) < RGB_TIMEOUT;

    if (timer_elapsed32(last_sync) > 250) {
        needs_sync = true;
    } else if (memcmp(&is_rgb_enabled, &last_state, sizeof(last_state))) {
        needs_sync = true;
        memcpy(&last_state, &is_rgb_enabled, sizeof(last_state));
    }

    // Perform the sync if requested
    if (needs_sync) {
        if (transaction_rpc_send(RPC_RGB_SYNC, sizeof(is_rgb_enabled), &is_rgb_enabled)) {
...
void rgb_sync_handler(uint8_t in_buflen, const void* in_data, uint8_t out_buflen, void* out_data) {
    memcpy(&is_rgb_enabled, in_data, in_buflen);
}
